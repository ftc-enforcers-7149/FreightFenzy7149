...
@Autonomous(name = "Blue WH Cycles THREE ONLY")
@Disabled
public class BlueCyclesTHREE extends Auto_V2_5 {
    ...
    private static final Trajectory driveToWall = MecanumDrive.trajectoryBuilder(preload.end(), Math.toRadians(170))
            .splineToSplineHeading(new Pose2d(-5, 80, Math.toRadians(95)), Math.toRadians(160))
    ...
    private void intake(double distanceFromWall) {
        ...
        customWait(() -> {
            ...
            if (deltaHeading(drive.getPoseEstimate().getHeading(), Math.toRadians(95)) < 0)  {
                drive.setWeightedDrivePower(new Pose2d(
                        Math.max(Math.min(
                                Math.pow(Math.max(144-drive.getPoseEstimate().getY()-distanceFromWall, 0), 2) * 0.00015 + 0.15,
            ...
            if (System.currentTimeMillis() > driveStartTime + 2000) return false;
    ...
    private void driveOutOfWarehouse() {
        ...
        customWait(() -> {
            ...
            if (drive.getPoseEstimate().getY() < 110) {
                drive.setWeightedDrivePower(new Pose2d(-0.6, 0.25, 0));
                intake.setIntakePower(0);
            }

            return System.currentTimeMillis() < driveStartTime + 1000 &&
                    drive.getPoseEstimate().getY() > 80;
        });
        drive.setWeightedDrivePower(new Pose2d());
